...
public class H2NotificationProvider implements NotificationProvider {
	...
	private class NotificationRowMapper implements RowMapper<Notification> {
		...
		private Level mapLevel(int ord) {
	...
	public boolean store(Notification n, boolean override) {
	...
	@Override
	public PageableResult<Notification> getNotifications(String userToken, int from, int limit) {
		PageableResult<Notification> n = new PageableResult<>();
	...
	@Override
	public NotificationSummary getSummary(String userToken) {
		...
		Level worst = null;
		if (r.get("m") != null) {
			int worstOrd = ((Number) r.get("m")).intValue();

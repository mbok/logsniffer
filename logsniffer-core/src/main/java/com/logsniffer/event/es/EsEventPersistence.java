...
public class EsEventPersistence implements EventPersistence {
	...
	public String persist(final Event event) {
		...
		try {
			String evStr = jsonMapper.writeValueAsString(event);
			final IndexRequest indexRequest = Requests.indexRequest(indexName)
					.type(getSnifferIdAsType(event.getSnifferId()))
					.source(evStr);
			String eventId = clientTpl.executeWithClient(
			...
			throw new DataAccessException("Failed to persiste event: "
					+ event.getId(), e);
		}
	}
	...
	public void delete(final long snifferId, final String[] eventIds) {
		clientTpl.executeWithClient(new ClientCallback<Object>() {
			...
			public Object execute(final Client client) {
				BulkRequest deletes = new BulkRequest();
				for (String id : eventIds) {
					deletes.add(new DeleteRequest(indexName,
							getSnifferIdAsType(snifferId), id));
				}
	...
	public void deleteAll(final long snifferId) {
		clientTpl.executeWithClient(new ClientCallback<Object>() {
			@Override
			public Object execute(final Client client) {
				logger.debug("Going to delete all events for sniffer: {}",
						snifferId);
				try {
					client.admin().indices().prepareDeleteMapping(indexName)
							.setType(getSnifferIdAsType(snifferId)).execute()
							.actionGet();
				} catch (TypeMissingException e) {
					logger.info(
							"Catched TypeMissingException when deleting all events of sniffer: {}",
							snifferId);
				}
	...
	private abstract class EsBaseEventsNativeQueryBuilder<BuilderType extends BaseEventQueryBuilder<?>>
		 ...
		 * @param snifferId
		 */
		public EsBaseEventsNativeQueryBuilder(final long snifferId,
		...
		@Override
		public BuilderType withEventCountTimeHistogram(
				final int maxHistogramIntervalSlots) {
		...
		public EventsResult list() {
			return clientTpl
					.executeWithClient(new ClientCallback<EventsResult>() {
						@Override
		...
		protected SearchRequestBuilder getBaseRequestBuilder(
				final Client esClient) {
			SearchRequestBuilder requestBuilder = esClient.prepareSearch(
					indexName).setTypes(getSnifferIdAsType(snifferId));
			requestBuilder
					.setFrom(offset)
					.setSize(size)
					.addSort(
							SortBuilders.fieldSort("occurrence")
									.order(SortOrder.ASC).ignoreUnmapped(true));
			return requestBuilder;
		}

		protected abstract SearchRequestBuilder adaptRequestBuilder(
				final Client esClient, final SearchRequestBuilder requestBuilder);
		...
		private EventsResult list(final Client esClient) {
			...
			if (maxHistogramIntervalSlots > 0) {
				StatsBuilder timeRangeAgg = AggregationBuilders.stats(
						"timeRange").field("occurrence");
				SearchRequestBuilder timeRangeQuery = adaptRequestBuilder(
						esClient, getBaseRequestBuilder(esClient).setSize(0)
								.addAggregation(timeRangeAgg));
				try {
					Stats timeRangeStats = timeRangeQuery.execute().actionGet()
							.getAggregations().get("timeRange");
					long timeRange = (long) (timeRangeStats.getMax() - timeRangeStats
							.getMin());
					logger.debug("Time range query: {}", timeRangeQuery);
					logger.debug(
							"Retrieved time range for events of sniffer={} in {}ms: {}",
							snifferId, System.currentTimeMillis() - start,
							timeRange);
					histogram = new EventsCountHistogram();
					Interval interval = getInterval(timeRange,
							maxHistogramIntervalSlots, histogram);
					requestBuilder.addAggregation(AggregationBuilders
							.dateHistogram("eventsCount").interval(interval)
							.field("occurrence").order(Order.KEY_ASC));
				} catch (SearchPhaseExecutionException e) {
					logger.warn(
							"Events histogram disabled because of exceptions (probably no events?)",
							e);
				}
			}
			SearchResponse response = requestBuilder.execute().actionGet();
			ArrayList<AspectEvent> events = new ArrayList<EventPersistence.AspectEvent>();
			for (SearchHit h : response.getHits().getHits()) {
				try {
					AspectEventImpl event = jsonMapper.readValue(
							h.getSourceAsString(), AspectEventImpl.class);
					event.setId(h.getId());
					events.add(event);
				} catch (Exception e) {
					throw new DataAccessException("Failed to decode event: "
							+ h.getSourceAsString(), e);
				}
			}
			if (histogram != null) {
				histogram
						.setEntries(new ArrayList<EventPersistence.HistogramEntry>());
				for (DateHistogram.Bucket e : ((DateHistogram) response
						.getAggregations().get("eventsCount")).getBuckets()) {
					histogram.getEntries().add(
							new HistogramEntry(e.getKeyAsNumber().longValue(),
									e.getDocCount()));
				}
			}
			logger.debug(
					"Retrieved events for sniffer={} in {}ms with query: {}",
					snifferId, System.currentTimeMillis() - start,
					requestBuilder);
			return new EventsResult(response.getHits().totalHits(), events,
					histogram);
		}
	}

	private class EsEventsNativeQueryBuilder extends
			EsBaseEventsNativeQueryBuilder<NativeQueryBuilder> implements
			NativeQueryBuilder {
		private String nativeQuery;

		public EsEventsNativeQueryBuilder(final long snifferId,
				final int offset, final int size) {
		...
		@Override
		protected SearchRequestBuilder adaptRequestBuilder(
				final Client esClient, final SearchRequestBuilder requestBuilder) {
			return requestBuilder.setExtraSource(nativeQuery);
		}
	}

	private class EsEventQueryBuilder extends
			EsBaseEventsNativeQueryBuilder<EventQueryBuilder> implements
			EventQueryBuilder {

		public EsEventQueryBuilder(final long snifferId, final int offset,
				final int size) {
			super(snifferId, offset, size);
		}
		...
		@Override
		public EventQueryBuilder withEntryFieldsMapAspect(
				final FieldsProjection[] fields, final EntriesJoinType joinType) {
			// Not supported
			return this;
		}
		...
		protected SearchRequestBuilder adaptRequestBuilder(
			...
			if (from != null || to != null) {
				RangeFilterBuilder occRange = FilterBuilders
						.rangeFilter("occurrence");
				if (from != null) {
			...
			if (filter != null) {
				requestBuilder.setQuery(QueryBuilders.filteredQuery(
	...
	@Override
	public EventQueryBuilder getEventsQueryBuilder(final long snifferId,
			final long offset, final int limit) {
		return new EsEventQueryBuilder(snifferId, (int) offset, limit);
	}

	@Override
	public NativeQueryBuilder getEventsNativeQueryBuilder(final long snifferId,
			final long offset, final int limit) {
		return new EsEventsNativeQueryBuilder(snifferId, (int) offset, limit);
	}

	protected Interval getInterval(final long timeRange,
			final int maxSlotsCount, final EventsCountHistogram histogram) {
	...
	public Event getEvent(final long snifferId, final String eventId) {
		return clientTpl.executeWithClient(new ClientCallback<Event>() {
			...
			public Event execute(final Client client) {
				try {
					GetResponse r = client
							.prepareGet(indexName,
									getSnifferIdAsType(snifferId), eventId)
							.execute().get();
					if (r != null && r.isExists()) {
						Event event = jsonMapper.readValue(
								r.getSourceAsString(), Event.class);
						event.setId(r.getId());
						return event;
					} else {
						return null;
					...
					throw new DataAccessException("Failed to load for sniffer="
							+ snifferId + " the event: " + eventId, e);
				}
			}
		});
	}
	...
	public static class AspectEventImpl extends Event implements AspectEvent {
		...
		@Override
		public <AspectType> void setAspect(final String aspectKey,
		...
		@SuppressWarnings("unchecked")
		@Override
		public <AspectType> AspectType getAspect(final String aspectKey,
	...
	public AspectProvider<AspectSniffer, Integer> getEventsCounter() {
		return new PostAspectProvider<SnifferPersistence.AspectSniffer, Integer>() {
			...
			public void injectAspect(final List<AspectSniffer> hosts) {
				...
				HashMap<String, AspectSniffer> mapHosts = new HashMap<String, SnifferPersistence.AspectSniffer>();
				final long[] hostIds = new long[hosts.size()];
				int i = 0;
				for (AspectSniffer s : hosts) {
					hostIds[i++] = s.getId();
					mapHosts.put(Long.toString(s.getId()), s);
					s.setAspect(EVENTS_COUNT, 0);
				}
				TermsFacet countersFacet = clientTpl
						.executeWithClient(new ClientCallback<TermsFacet>() {
							@Override
							public TermsFacet execute(final Client client) {
								...
								SearchResponse response = requestBuilder
										.execute().actionGet();
								logger.debug(
										"Performed events counting search {} in {}ms",
										requestBuilder,
										System.currentTimeMillis() - start);
								return response.getFacets().facet(
										TermsFacet.class, "eventsCounter");
							}
						});

				for (org.elasticsearch.search.facet.terms.TermsFacet.Entry e : countersFacet
						.getEntries()) {
					String id = e.getTerm().string();

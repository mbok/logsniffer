...
public class LogEntriesRestController {
	private static Logger logger = LoggerFactory
			.getLogger(LogEntriesRestController.class);
	@Autowired
	private LogSourceProvider logsSourceProvider;

	private LogSource<LogInputStream> getActiveLogSource(final long logSourceId)
	...
	LogEntriesResult getEntries(
			@Valid @RequestBody final LogSource<LogInputStream> activeLogSource,
			@RequestParam("log") final String logPath,
			@RequestParam(value = "mark", required = false) final String mark,
			@RequestParam(value = "count") final int count) throws IOException,
			FormatException, ResourceNotFoundException {
		logger.debug(
				"Start load entries log={} from source={}, mark={}, count={}",
				logPath, activeLogSource, mark, count);
		try {
			Log log = getLog(activeLogSource, logPath);
			LogPointer pointer = null;
			LogRawAccess<LogInputStream> logAccess = activeLogSource
					.getLogAccess(log);
			...
			if (count > 0) {
				BufferedConsumer bc = new BufferedConsumer(count);
				activeLogSource.getReader().readEntries(log, logAccess,
						pointer, bc);
				return new LogEntriesResult(activeLogSource.getReader()
						.getFieldTypes(), bc.getBuffer());
			} else {
				return new LogEntriesResult(activeLogSource.getReader()
						.getFieldTypes(), new BackwardReader(
						activeLogSource.getReader()).readEntries(log,
						logAccess, pointer, count));
			}

		} finally {
			logger.debug("Finished log entries from log={} and source={}",
					logPath, activeLogSource);
		}
	}
	...
	@ResponseBody
	LogEntriesResult getEntries(
			@PathVariable("logSource") final long logSource,
			@RequestParam("log") final String logPath,
			@RequestParam(value = "mark", required = false) final String mark,
			@RequestParam(value = "count") final int count) throws IOException,
			FormatException, ResourceNotFoundException {
		logger.debug(
				"Start load entries log={} from source={}, mark={}, count={}",
				logPath, logSource, mark, count);
		try {
			LogSource<LogInputStream> activeLogSource = getActiveLogSource(logSource);
	...
	LogEntriesResult getRandomAccessEntries(
			@Valid @RequestBody final LogSource<LogInputStream> activeLogSource,
			@RequestParam("log") final String logPath,
			@RequestParam(value = "mark") final String mark)
			throws IOException, FormatException, ResourceNotFoundException {
		logger.debug(
				"Start loading random access entries log={} from source={}, mark={}",
				logPath, activeLogSource, mark);
		try {
			Log log = getLog(activeLogSource, logPath);
			LogRawAccess<LogInputStream> logAccess = activeLogSource
					.getLogAccess(log);
			...
			BufferedConsumer bc = new BufferedConsumer(
					SourcesController.DEFAULT_ENTRIES_COUNT + 1);
			if (pointer != null) {
			...
			activeLogSource.getReader().readEntries(
					log,
					logAccess,
					pointer != null ? logAccess.createRelative(pointer, -1)
							: null, bc);
			List<LogEntry> entries = bc.getBuffer();
			if (entries.size() > 0) {
				LogEntry first = entries.get(0);
				LogEntry last = entries.get(entries.size() - 1);
				if (!first.getStartOffset().isSOF()
						&& last.getEndOffset().isEOF() && entries.size() < 10) {
					// Hm, EOF reached
					return new LogEntriesResult(activeLogSource.getReader()
							.getFieldTypes(), new BackwardReader(
							activeLogSource.getReader()).readEntries(log,
							logAccess, last.getEndOffset(), -10));
				} else if (logAccess.getDifference(pointer,
						first.getStartOffset()) == 0) {
					// -1 without effect, return from beginning
					return new LogEntriesResult(activeLogSource.getReader()
							.getFieldTypes(), entries);
				} else {
			...
			logger.debug(
					"Finished loading random access entries from log={} and source={}",
					logPath, activeLogSource);
		}
	}
	...
	public static class SearchResult {
		...
		private LogPointerTransfer lastPointer;
		 ...
		 */
		public LogPointerTransfer getLastPointer() {
			return lastPointer;
		}
	...
	SearchResult searchEntries(@RequestBody @Valid final Scanner scanner,
			...
			final BindingResult bResult) throws IOException, FormatException,
			ResourceNotFoundException {
		long start = System.currentTimeMillis();
		logger.debug(
				"Start searching entries log={} from source={}, mark={}, count={}",
				logPath, logSource, mark, count);
		LogSource<LogInputStream> source = getActiveLogSource(logSource);
		Log log = getLog(source, logPath);
		LogRawAccess<LogInputStream> logAccess = source.getLogAccess(log);
		...
		scanner.find(reader, new TimeoutReaderStrategy(3 * 1000) {
			...
			public boolean continueReading(final Log log,
				...
				return !searchResult.sofReached
						&& searchResult.lastPointer == null
						&& super.continueReading(log, pointerFactory,
								currentReadEntry);
			}
			...
			public void consume(final EventData eventData) throws IOException {
				...
				searchResult.lastPointer = eventData.getEntries().get(0)
						.getStartOffset();
			}
		});
		...
		if (searchResult.lastPointer != null) {
			// Found
			logger.debug("Found next entry of interest in {} at: {}", log,
					searchResult.lastPointer);
			BufferedConsumer bc = new BufferedConsumer(Math.abs(count));
			source.getReader().readEntries(log, logAccess,
					(LogPointer) searchResult.lastPointer, bc);
			searchResult.entries = new LogEntriesResult(source.getReader()
					.getFieldTypes(), bc.getBuffer());
		...
		searchResult.scannedSize = Math.abs(logAccess.getDifference(
				searchPointer, (LogPointer) searchResult.lastPointer));
		return searchResult;
	}

	private Log getLog(final LogSource<LogInputStream> activeLogSource,
